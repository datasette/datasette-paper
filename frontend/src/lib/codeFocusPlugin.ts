/**
 * Tracks which single `code_block` (if any) is "active" — i.e. mounted as a
 * live CodeMirror editor (tier 1). See plans/codemirror/02-design.md §4.
 *
 * A block is a *candidate* when the view is editable, the PM selection sits
 * wholly inside a `code_block`, and the view isn't mid-composition. On a new
 * candidate the plugin kicks `loadCmCore()` plus the block language's `cm()`
 * loader; the block stays a plain PM-editable `<pre>` (typing is never locked
 * out) until *both* resolve, at which point a `codeFocusKey` meta flips the
 * plugin's `active` position and stashes the resolved `core` + `support`.
 * Selection leaving the block deactivates immediately. At most one block is
 * active; moving between two code blocks tears one CM down and mounts the
 * other (cheap once chunks are cached).
 *
...
/** The code_block the selection sits wholly inside (returns its `getPos`),
 * gated on editability and composition, or null. */
    ...
    view(editorView) {
      ...
      return {
        update(view) {
          ...
          const entry = node
            ? resolveLanguage(node.attrs.language as string | null)
            : undefined;
          Promise.all([
            loadCmCore(),

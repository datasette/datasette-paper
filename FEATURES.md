...
| `profile-papers` | Profile "Papers" section — the `listProfileDocs` read over the `doc-activity` rollup (profile actor's created/recently-edited docs ∩ viewer's visible set); endpoint + web component land in the follow-up PR | `datasette_paper/sql/queries.sql` |

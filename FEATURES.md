...
| `task-assign` | @mentions in a task item assign it (subtree-inheriting, own-overrides); a date atom is its due date. Pure doc interpretation via `extract_tasks`; a write-tail m009 index + `/todos` API + profile & `/-/paper/todos` surfaces expose it cross-doc | `datasette_paper/markdown.py` |

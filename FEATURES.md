...
| `tui` | Terminal client (`datasette paper tui <url>`) — a fourth client of the HTTP + SSE protocol, shipped behind the `[tui]` extra: `PaperClient`/`DocSession` boot + client-side materialize via the package's own `pm_schema`, a hand-rolled SSE reader, and client-side `ReplaceStep` submission with 409/410/400/422 semantics | `datasette_paper/tui/client.py` |

...
| `profile-papers` | Data layer for the profile "Papers" section — the `_datasette_paper_doc_activity` per-(doc, actor) last-edited rollup: upserted on every accepted step at both step-insert sites, survives step compaction, backfilled by m008, purged on doc hard-delete | `datasette_paper/migrations.py` |

...
| `breadcrumbs` | Header-bar breadcrumb on every paper page — "[icon] Papers / <current>" replaces Datasette's "home" crumb (index: just "Papers"; tag: `#tag`; doc: the doc name), and the doc-page crumb + tab title update live on rename (own commit + a `renamed` SSE broadcast to collaborators) | `datasette_paper/templates/paper_base.html` |

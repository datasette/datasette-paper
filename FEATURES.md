...
| `source` | A named, parameterless SQL query block (fenced `source name=NAME db=DB`) that inline `value` atoms reference; its SQL surface upgrades to a CodeMirror editor on focus (a collapsed pill declines the mount) | `frontend/src/lib/sourceBlockView.ts` |

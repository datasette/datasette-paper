...
| `date` | Inline date atom — `{date, time?, tz?, format?}`, slash-menu insert (`/date` + quick `/today`,`/tomorrow`,`/yesterday`) + click-to-edit natural-language popup with a per-node format picker (strftime presets + custom); a neutral chip that tints overdue/today only inside an unchecked task; round-trips as a `paper:/date/` link | `frontend/src/lib/dateView.ts` |

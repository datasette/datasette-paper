...
| `copy-markdown` | Doc-header ⋯ → "Copy as markdown" — serializes the live doc through the client serializer (lazy-loaded prosemirror-markdown; full schema coverage enforced by a parity test) onto the clipboard, flashing ✓ Copied / ✗ Failed | `frontend/src/lib/markdownSerializer.ts` |
...
| `task-assign` | @mentions in a task item assign it (subtree-inheriting, own-overrides); a date atom is its due date. Pure doc interpretation via `extract_tasks`; a write-tail m009 index + viewer-acl-filtered `/todos` API expose it cross-doc, rendered read-only by the `<profile-todos>` profile section and the bucketed `/-/paper/todos` page | `datasette_paper/markdown.py` |
...
| `callout` | GitHub-style admonition block (`> [!NOTE]` family) — a title + body, top-level-only (not in the `block` group; `doc` content overridden to `(block \| callout)+`), five closed kinds clamped defensively, round-trips as a `> [!KIND] Title` marker + `> `-prefixed body. Foldable via a chevron: a shared `collapsed` attr hides the body down to the title and round-trips as the Obsidian `[!KIND]-` suffix (read-only viewers fold locally, no step) | `frontend/src/lib/calloutView.ts` |
| `link-graph` | force-directed view of the viewable `[[wikilink]]` graph, colored/filtered by tag/state/kind; a shared `GraphModal` also hosts a doc-centred ego view (BFS to an adjustable depth around a focus doc, `?focus=` echo for zero-link docs) | `frontend/src/lib/LinkGraph.svelte` |
| `breadcrumbs` | Header-bar breadcrumb on every paper page — "home / [icon] Papers / <current>" (home via Datasette's permission-gated crumb_items; index: no current segment; tag: `#tag`; doc: the doc name). On doc pages the current segment self-links and a chevron opens a GitHub-style searchable switcher of viewable papers (updated_at DESC); the crumb + tab title update live on rename (own commit + a `renamed` SSE broadcast to collaborators) | `datasette_paper/templates/paper_base.html` |

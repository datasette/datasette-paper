...
| `breadcrumbs` | Header-bar breadcrumb on every paper page — "home / [icon] Papers / <current>" (home via Datasette's permission-gated crumb_items; index: no current segment; tag: `#tag`; doc: the doc name), and the doc-page crumb + tab title update live on rename (own commit + a `renamed` SSE broadcast to collaborators) | `datasette_paper/templates/paper_base.html` |
